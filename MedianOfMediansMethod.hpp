...
int medianOfMediansHelper(std::vector<int>& a, std::vector<int>::iterator low, std::vector<int>::iterator high) {
    ...
    for (int i = 0; i < groups; ++i) {
        ...
        int groupRight = std::min(groupLeft + 4, *high);
        int groupMedian = InsertionSortMedian(groupLeft, groupRight);
...
const int & medianofMedians(std::vector<int> & a, int left, int right){
